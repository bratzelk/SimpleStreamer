...
/*
 * A NewBlock instruction contains the actual bytes of the block. It should be sent only when the
 * Server does not actually have the bytes already. Usually this is when the file is being
 * synchronised for the first time, or when changes occur that need to be sent to the server. A
 * CopyBlock can be upgraded to a NewBlock instruction using the constructor.
 *
 * (In fact, both instructions contain the actual bytes, but the marshalling to JSON does not
 * include the bytes for the CopyBlock.)
 */
...
public class NewBlockInstruction extends Instruction {
  ...
  public NewBlockInstruction(CopyBlockInstruction cb) {
