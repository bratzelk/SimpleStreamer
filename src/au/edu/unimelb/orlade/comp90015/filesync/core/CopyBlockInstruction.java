...
/*
 * A CopyBlock instruction does not contain the actual bytes of the block. Rather it assumes the
 * bytes are already at the server, from a previous synchronisation step. The instruction merely
 * sends a hash of the bytes. If the file at the server has changed by other processes, causing the
 * bytes to be actually not available, then the CopyBlock needs to be upgraded to a NewBlock that
 * contains the actual bytes and this needs to be sent.
 */
