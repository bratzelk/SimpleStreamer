...
/*
 * This test thread provides comments to explain how the Client/Server architecture could be
 * implemented that uses the file synchronisation protocol.
 */
  ...
  public void run() {
    ...
    while ((inst = fromFile.NextInstruction()) != null) {
      ...
      try {
        ...
        try {
          /*
           * At this point the Server needs to send a request back to the Client to obtain the
           * actual bytes of the block.
           */
      ...
      /*
       * If using a synchronous RequestReply protocol, the server can now acknowledge that the block
       * was correctly received, and the next instruction can be sent.
       */
